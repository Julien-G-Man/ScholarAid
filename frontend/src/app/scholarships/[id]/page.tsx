...
async function getScholarship(id: string): Promise<Scholarship | null> {
  try {
    const res = await fetch(`${API_URL}/scholarships/${id}/`, { next: { revalidate: 300 } });
  ...
  return (
    <div className="container py-5">
        ...
        <div className="col-lg-4">
          {scholarship.logo_url && (
            <div className="d-flex flex-column align-items-center mb-4">
              ...
              <img src={scholarship.logo_url} alt={`${scholarship.name} Logo`} style={{ maxHeight: '150px', objectFit: 'contain' }} className="img-fluid mb-3" />
            </div>
          )}
          <ul className="list-group list-group-flush">
            <li className="list-group-item border-0 px-0"><strong>Provider:</strong> {scholarship.provider}</li>
            <li className="list-group-item border-0 px-0"><strong>Institution:</strong> {scholarship.institution || 'N/A'}</li>
            <li className="list-group-item border-0 px-0"><strong>Level:</strong> {scholarship.level || 'N/A'}</li>
            <li className="list-group-item border-0 px-0"><strong>Deadline:</strong> {formatDate(scholarship.deadline)}</li>
          </ul>
        </div>
      </div>
      ...
      <div className="text-center mt-4 d-flex gap-3 justify-content-center">
        {scholarship.link && (
          <a href={scholarship.link} target="_blank" rel="noopener noreferrer" className="btn btn-primary-brand btn-lg rounded-pill">
            Visit Official Page
          </a>
        )}
